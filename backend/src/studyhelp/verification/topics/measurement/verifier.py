"""Deterministic verifier for NCERT Class 5 Ch.14 ("How Big? How Heavy?" --
unit conversion for length/weight/volume). Mirrors `DecimalsVerifier`'s /
`AreaPerimeterVerifier`'s pattern exactly (ARCHITECTURE.md D41): no
...
reject rather than a low-confidence passthrough (D40's rationale).
"""
...
from studyhelp.verification.topics.measurement.step_checkers import (
    STEP_TYPE_FIELD_MODELS,
    compare_to_expected,
)
from studyhelp.verification.topics.measurement.sympy_utils import check_final_value
...
class MeasurementVerifier:
    ...
    def _resolve_near_match(
        ...
        # Same rationale as fractions_addition's D40 and every subsequent
        # free-text topic's precedent: `evaluated` only ever contains
    ...
    def _cross_check_final_value(self, problem: Problem, node: StepNode) -> None:
        ...
        if value is None:
            return
        given_value = problem.given["value"]
        direction = problem.given["direction"]
        factor = problem.given["factor"]
        if not check_final_value(given_value, direction, factor, value):
            raise ValueError(
                f"Problem '{problem.problem_id}' data integrity error: "
                f"{given_value} {direction} {factor} != {value} per independent sympy cross-check"
            )
