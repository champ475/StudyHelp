"""Deterministic verifier for NCERT Class 5 Ch.13 ("Ways to Multiply and
Divide"). Mirrors `DecimalsVerifier`'s / `AreaPerimeterVerifier`'s pattern
exactly (ARCHITECTURE.md D41): no client-declared `step_type`, tries every
step type's grammar against the submitted text, restricts candidate search
to the DAG frontier (this topic's DAG is linear with no step-type
recurrence -- a given problem's graph only ever contains the
multiplication step types OR the division step types, never both), and
rejects any non-exact frontier-parseable match rather than a
...
from studyhelp.verification.topics.multiplication_division.step_checkers import (
    STEP_TYPE_FIELD_MODELS,
    compare_to_expected,
)
from studyhelp.verification.topics.multiplication_division.sympy_utils import check_final_value

_EvaluatedCandidate = tuple[StepNode, list[FieldDiscrepancy], float, dict[str, object]]


class MultiplicationDivisionVerifier:
    ...
    def _resolve_near_match(self, evaluated: list[_EvaluatedCandidate]) -> VerifyResult:
        ...
        # / area_perimeter's precedent: `evaluated` only ever contains
        # candidates whose text already parsed cleanly into that step
        # type's exact grammar, so a non-exact value match is unambiguous,
    ...
    def _cross_check_final_value(self, problem: Problem, node: StepNode) -> None:
        ...
        if not check_final_value(a, b, op, value):
            raise ValueError(
                ...
                f"{a} {op} {b} != {value} per independent sympy cross-check"
            )
