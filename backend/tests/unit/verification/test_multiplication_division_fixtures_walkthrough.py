"""Walks every seeded multiplication_division problem fixture end-to-end
through the real verifier, submitting each node's own `expected_state`
(rendered as the free text a student would actually type) in canonical
order. Strongest available check that hand-authored fixture arithmetic is
self-consistent — mirrors the other topics' fixtures-walkthrough pattern."""
...
def _render(node: StepNode) -> str:
    ...
    if node.type == "multiply_units":
        return f"{state['digit']} x {state['multiplier']} = {state['product']}"
    if node.type == "multiply_tens":
        return f"{state['digit']} x {state['multiplier']} + {state['carry_in']} = {state['product']}"
    if node.type in ("divide_tens", "divide_units"):
        return f"{state['dividend_group']} / {state['divisor']} = {state['quotient_digit']} remainder {state['remainder']}"
...
def test_canonical_path_walks_cleanly_to_final_answer(problem: Problem) -> None:
    verifier = MultiplicationDivisionVerifier()
    accepted: list[str] = []
...
def test_final_answer_matches_raw_arithmetic(problem: Problem) -> None:
    ...
    if op == "/":
        assert a % b == 0, f"{problem.problem_id}: division must be exact (no remainder)"
