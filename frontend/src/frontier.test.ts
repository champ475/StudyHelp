...
import { computeFrontier, frontierStepTypes } from "./frontier";
...
const problem: PublicProblem = {
  ...
  step_graph: [
    { step_id: "s1", type: "compare_column", next: ["s2"] },
    { step_id: "s2", type: "borrow", next: ["s3"] },
    { step_id: "s3", type: "subtract_column", next: [] },
  ],
  alt_paths: [],
};

describe("computeFrontier / frontierStepTypes", () => {
  it("starts at the first node when nothing is accepted", () => {
    expect(frontierStepTypes(problem, [])).toEqual(["compare_column"]);
  });

  it("advances to the next node's type once a step is accepted", () => {
    expect(frontierStepTypes(problem, ["s1"])).toEqual(["borrow"]);
  });
  ...
  it("includes alt-path entries alongside the canonical first node", () => {
    ...
    expect(frontierStepTypes(withAlt, [])).toEqual(
